...
text\<open>The efficient and abstract @{term interp_op} algorithms map related concrete and abstract
...
using assms proof(induction log arbitrary: T t log1 log2 u U)
  case Nil
  assume 1: \<open>t \<preceq> T\<close> and 2: \<open>efficient_interp_op oper ([], t) = (log1, u)\<close>
    and 3: \<open>interp_op oper ([], T) = (log2, U)\<close>
...
next
  ...
  assume 1: \<open>t \<preceq> T\<close> and 2: \<open>efficient_interp_op oper (logop # logops, t) = (log1, u)\<close>
    and 3: \<open>interp_op oper (logop # logops, T) = (log2, U)\<close>
    and IH: \<open>(\<And>T t log1 log2 u U. t \<preceq> T \<Longrightarrow> efficient_interp_op oper (logops, t) = (log1, u) \<Longrightarrow>
                interp_op oper (logops, T) = (log2, U) \<Longrightarrow> log1 = log2 \<and> u \<preceq> U)\<close>
  {
    ...
    obtain action1 and action1' and u' and u'' and u''' where 5: \<open>efficient_undo_op (logop, t) = u'\<close> and
        6: \<open>efficient_interp_op oper (logops, u') = (action1, u'')\<close> and
    ...
    obtain action2 and action2' and U' and U'' and U''' where 8: \<open>undo_op (logop, T) = U'\<close> and
        9: \<open>interp_op oper (logops, U') = (action2, U'')\<close> and
