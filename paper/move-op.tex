...
For example:
\begin{itemize}
    \item Most \textbf{filesystems} are organised as a tree with directories as branch nodes and files as leaf nodes.
        % In Unix-style filesystems, the nodes of the tree are known as \emph{inodes}.
        Strictly speaking, filesystems with hardlinks form a DAG; we will focus on trees now and show in \S\ref{sec:hardlinks} how to add support for links.
...
\begin{itemize}
    ...
    \item We formalise an abstract algorithm using Isabelle/HOL~\cite{DBLP:conf/tphol/WenzelPN08} and obtain a computer-checked proof of correctness of the algorithm.
    ...
    \item To demonstrate the practical viability of our approach, we refine our abstract algorithm to an efficient implementation within Isabelle/HOL and prove the equivalence of the two.
        We then extract a formally verified, executable Scala implementation from the Isabelle/HOL definitions and evaluate its performance with replicas across three continents.
...
Intuitively, when a replica needs to apply an operation with timestamp \isa{t}, it first undoes the effect of any operations with a timestamp greater than \isa{t}, then performs the new operation, and finally re-applies the previously undone operations.
...
For space reasons we give only a brief sketch of the proofs in this paper, but the full details can be found in our open source Isabelle files~\cite{SourceFiles}.
...
Proving it is easy because each of the \isa{do\_op}, \isa{undo\_op} and \isa{redo\_op} functions individually maintains the invariant \isa{unique\_parent tree}, and thus \isa{apply\_op} also maintains the invariant.
...
To prove \isa{apply\_op} preserves acyclicity we show that trees obtained from \isa{apply\_ops} can be built from repeated use of \isa{do\_op} invocations alone, without \isa{undo\_op} or \isa{redo\_op}.
(This is the case when operations are applied in order of increasing timestamp.)
Then, from our previous results, any tree produced by \isa{apply\_ops} is acyclic.
...
We refer the reader to our Isabelle theory files~\cite{SourceFiles}.
...
Whilst these constructs are convenient for theorem proving, they do not translate well to efficient code.
...
The \isa{hashable} type class on keys means that keys must have an associated hashing function.
...
We use the hash-map implementation from the Isabelle Collections Framework~\cite{DBLP:conf/itp/LammichL10}.
...
We produce a new set of definitions---mirroring the structure of our existing ones---wherein we replace all occurrences of the $(\isa{'n} \times \isa{'m} \times \isa{'n})\ \isa{set}$ type, and any associated set operations, with the $(\isa{'n}, \isa{'m} \times \isa{'n})\ \isa{hm}$ type and associated hash-map operations.
For clarity, we refer to any definition using hash-maps as an ``executable'' definition, and any definition making use of the \isa{set} type as an ``abstract'' definition.
...
However \isa{ancestor} is different to other definitions as it is an inductively defined relation rather than a recursive function, and we need further work to make this executable.
We characterise this relation with the following:

% DPM: add efficient_ancestor_simp here
\begin{isabelle}
\isacommand{theorem}\isamarkupfalse%
\ executable{\isacharunderscore}ancestor{\isacharunderscore}simp\ {\isacharbrackleft}code{\isacharbrackright}{\isacharcolon}\isanewline
\ \ \isakeyword{shows}\ {\isacartoucheopen}executable{\isacharunderscore}ancestor\ t\ p\ c\ {\isasymlongleftrightarrow}\isanewline
...
\ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ a\ {\isacharequal}\ p\ {\isasymor}\ executable{\isacharunderscore}ancestor\ t\ p\ a{\isacharparenright}{\isacartoucheclose}
\end{isabelle}

Note that \isa{executable\_ancestor\_simp} is tagged with a $\isa{code}$ ``attribute'', a hint that this theorem should be used as a transformation during code generation.
Isabelle/HOL takes this theorem as the definition, for code generation, of the \isa{executable\_ancestor} relation and is able to translate this to a recursive function definition in each target backend---this transformation is ``semantics preserving'', as the characterisation was proved as a theorem within HOL.

We demonstrate the correctness of our executable definitions against our abstract definitions.
The properties proved are best described using an example, as they all follow a similar pattern.
We use the lemma \isa{executable\_do\_op\_simulates} as our exemplar:
...
\ \ \ \ \isakeyword{and}\ {\isadigit{2}}{\isacharcolon}\ {\isacartoucheopen}executable{\isacharunderscore}do{\isacharunderscore}op\ {\isacharparenleft}oper{\isacharcomma}\ t{\isacharparenright}\ {\isacharequal}\ {\isacharparenleft}log{\isadigit{1}}{\isacharcomma}\ u{\isacharparenright}{\isacartoucheclose}\isanewline
...
We show that the \isa{executable\_do\_op} and the \isa{do\_op} functions are in ``lock-step'', mapping related trees $t \preceq T$ to related trees $u \preceq U$ and producing identical logs of operations whenever they are applied to the same input operation, \isa{oper}.
This lock-step is repeated bottom-up for every executable-abstract algorithm pairs, until we reach the final correctness theorem for \isa{apply\_ops}:
...
\ \ \isakeyword{assumes}\ {\isadigit{1}}{\isacharcolon}\ {\isacartoucheopen}executable{\isacharunderscore}apply{\isacharunderscore}ops\ opers\ {\isacharequal}\ {\isacharparenleft}log{\isadigit{1}}{\isacharcomma}\ u{\isacharparenright}{\isacartoucheclose}\isanewline
...
That is, if both algorithms are applied to the same operations then they produce identical logs and trees that contain the same set of bindings---i.e. the two trees are extensionally equivalent, despite having in-memory representations.
Moreover, we have corollaries of \isa{apply\_ops\_commutes} and \isa{apply\_ops\_acyclic}, respectively, further establishing the correctness of our executable algorithms:\footnote{The ``executable'' analogue of \isa{apply\_ops\_unique\_parent} is trivially true, as the output tree is stored as a hash-map, and therefore too trivial to state as a theorem.}:

% DPM: add efficient_apply_ops_commutes and efficient_apply_ops_acyclic here
\begin{isabelle}
\isacommand{theorem}\isamarkupfalse%
\ executable{\isacharunderscore}apply{\isacharunderscore}ops{\isacharunderscore}commutes{\isacharcolon}\isanewline
...
\ \ \ \ \isakeyword{and}\ {\isadigit{4}}{\isacharcolon}\ {\isacartoucheopen}executable{\isacharunderscore}apply{\isacharunderscore}ops\ ops{\isadigit{1}}\ {\isacharequal}\ {\isacharparenleft}log{\isadigit{1}}{\isacharcomma}\ t{\isacharparenright}{\isacartoucheclose}\isanewline
\ \ \ \ \isakeyword{and}\ {\isadigit{5}}{\isacharcolon}\ {\isacartoucheopen}executable{\isacharunderscore}apply{\isacharunderscore}ops\ ops{\isadigit{2}}\ {\isacharequal}\ {\isacharparenleft}log{\isadigit{2}}{\isacharcomma}\ u{\isacharparenright}{\isacartoucheclose}\isanewline
...
\isanewline
\isacommand{theorem}\isamarkupfalse%
\ executable{\isacharunderscore}apply{\isacharunderscore}ops{\isacharunderscore}acyclic{\isacharcolon}\isanewline
\ \ \isakeyword{assumes}\ {\isadigit{1}}{\isacharcolon}\ {\isacartoucheopen}executable{\isacharunderscore}apply{\isacharunderscore}ops\ ops\ {\isacharequal}\ {\isacharparenleft}log{\isacharcomma}\ t{\isacharparenright}{\isacartoucheclose}\isanewline
\ \ \isakeyword{shows}\ {\isacartoucheopen}{\isasymnexists}n{\isachardot}\ executable{\isacharunderscore}ancestor\ t\ n\ n{\isacartoucheclose}
\end{isabelle}

Note the statement of \isa{executable\_apply\_ops\_commutes} differs from \isa{apply\_ops\_commutes}---the latter asserts identical sets are obtained from convergence, whilst the former asserts that reading any key binding in $t$ gives the same result as a reading in $u$.
Whilst this is weaker than that of \isa{apply\_ops\_acyclic}, it is the strongest statement one can hope for: the resulting hash-maps may contain the same key-value bindings but may not be structurally equal, depending on the ordering of operations.
...
We use a synthetic workload wherein each replica generates move operations at the same rate; for each move operation the generating replica chooses parent and child nodes uniformly at random from a set of 1,000 tree nodes.
...
A locally generated operation always has a timestamp greater than any existing operation at the generating replica at that time (from the use of Lamport timestamps), and thus applying that operation requires only a \isa{do\_op}, and no \isa{undo\_op} or \isa{redo\_op}, hence the near-constant execution time.
However, for remote operations, a number of undos and redos must be performed, depending on the degree of concurrency.
As the rate of operations increases, the time between successive operations becomes smaller than the network delay between replicas; thus, there are more operations ``in flight'' at the same time, and more calls to \isa{undo\_op} and \isa{redo\_op} are required order operations by timestamp.
...
A hand-optimised implementation of our algorithm could achieve higher throughput than our 24 operations/sec: despite \S\ref{subsect.extracting}, the code generated from our HOL definitions contains many optimisation opportunities due to its extraction from a theorem prover.
Nevertheless, our evaluation sends a positive signal about the practical applicability of our algorithm, and the generated code could serve as an executable ``golden model'' for validating other implementations.
...
Then, performing an operation requires waiting for synchronous communication with a server potentially located on a different continent, taking hundreds of milliseconds (Table~\ref{tab:rpc-times}); by contrast, a local operation taking a fraction of a millisecond is orders of magnitude faster.
