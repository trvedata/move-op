...
\author{Martin Kleppmann}{University of Cambridge}{mk428@cl.cam.ac.uk}{https://orcid.org/0000-0001-7252-6958}{Supported by a Leverhulme Trust Early Career Fellowship and by the Isaac Newton Trust.}

\author{Dominic P. Mulligan}{Arm Research, Cambridge}{Dominic.Mulligan@arm.com}{https://orcid.org/0000-0003-4643-3541}{}

\author{Victor B. F. Gomes}{Google\footnote{Work done while at the University of Cambridge}}{vb358@cl.cam.ac.uk}{https://orcid.org/0000-0002-2954-4648}{}

\author{Alastair R. Beresford}{University of Cambridge}{arb33@cl.cam.ac.uk}{https://orcid.org/0000-0003-0818-6535}{}

\authorrunning{M. Kleppmann, D.\,P. Mulligan, V.\,B.\,F. Gomes, and A.\,R. Beresford}
\Copyright{Martin Kleppmann, Dominic P. Mulligan, Victor B. F. Gomes, and Alastair R. Beresford}

\funding{The authors wish to acknowledge the support of The Boeing Company and the EPSRC ``REMS: Rigorous Engineering for Mainstream Systems'' programme grant (EP/K008528). Our evaluation was conducted using AWS credits from the AWS Educate program.}
\acknowledgements{Thank you to Marc Shapiro for feedback on a draft of this paper.}
...
\keywords{Conflict-free Replicated Data Types (CRDTs), distributed filesystems, formal verification}

\begin{document}
\maketitle
\begin{abstract}
    Replicated tree data structures are data models supporting distributed filesystems, such as Google Drive and Dropbox, and XML or JSON databases.
...
Many data management applications use a tree-structured or hierarchical data model.
For example, most file systems are trees (directories are branch nodes, and files are leaf nodes), and databases are used to store this directory structure and file metadata~\cite{Niazi:2017uf,Liu:2019zn}.\footnote{Strictly speaking, filesystems with hardlinks form a DAG; we will show in \S\ref{sec:extensions} how to support hardlinks.}
Moreover, XML databases and JSON document databases also operate on trees, which might represent rich text (a tree of paragraphs, lists, figures, sections, etc.), vector graphics, CAD drawings, and many other types of data.
Typically, a graphical user interface allows a user to edit this information interactively, resulting in updates to the underlying XML/JSON database: adding or deleting nodes in the tree, and moving nodes from one position in the tree to another.

When updating these trees, standard concurrency control techniques can be used, such as two-phase locking~\cite{Bernstein:1987va} or optimistic concurrency control~\cite{Cahill:2008eg}.
However, these techniques require synchronous coordination between replicas.
If we want the system to continue processing read and write requests even in the presence of arbitrary network partitions (in other words, if we want \emph{availability} and \emph{partition-tolerance} in the sense of the CAP theorem~\cite{Gilbert:2002il}), then we must avoid such coordination~\cite{Davidson:1985hv}.
Coordination avoidance has shown to be beneficial in several data systems~\cite{Bailis:2014th,AnnaKVS}, and network partitions are common~\cite{Alquraan:2018wj,Bailis:2014jx}.
...
Dropbox and Google Drive are widely-deployed examples of optimistically replicated filesystems: they run a daemon on the user's machine that watches a designated directory for changes, and locally maintains a database with the user's tree of files and directories.
...
In this paper we introduce a novel algorithm for handling concurrent updates to a replicated tree, such as a filesystem or XML database.
It allows replicas to manipulate the tree by creating new nodes, deleting nodes, or moving subtrees to a new location within the tree.
In order to rule out bugs like those in Google Drive we formally prove the correctness of our algorithm using the Isabelle/HOL proof assistant.
...
It is an example of a \emph{Conflict-free Replicated Data Type} or CRDT~\cite{Shapiro:2011wy,Shapiro:2011un}, and it guarantees a consistency model called \emph{strong eventual consistency}~\cite{Shapiro:2011un,Gomes:2017gy}.
...
\begin{tikzpicture}
  ...
  %%%
  \node [fill=red!15] at (-2.97,-2.4) {(\emph{a})};
  \node [fill=red!15] at (0.53,-2.4) {(\emph{b})};
  \node [fill=red!15] at (3.14,-2.4) {(\emph{c})};
  \node [fill=red!15] at (7.85,-2.4) {(\emph{d})};
  \node [rectangle,draw] at (-1.9,-3.5) {
      \begin{tikzpicture}[level distance=7mm]
...
\begin{tikzpicture}
  ...
  \node [rectangle,draw] (start) at (4.5,-3.5) {
      \begin{tikzpicture}[level distance=7mm]
  ...
  \node [rectangle,draw] (right) at (7.5,-3.5) {
      \begin{tikzpicture}[level distance=7mm]
      \useasboundingbox (-1.15,-2.3) rectangle (1.15,0.2);
...
As each replica propagates its operation to the other replica, a careless implementation might end up as shown in Figure~\ref{fig:move-cycle}a, in which $A$ and $B$ have formed a cycle, detached from the tree.
...
\citet{Najafzadeh:2017vk,Najafzadeh:2018bw} previously implemented a replicated filesystem with a move operation, and analysed the case of concurrent move operations introducing a cycle.
...
\end{isabellebody}
\caption{The move operation algorithm, implemented in the Isabelle/HOL language (continued in Figure~\ref{fig:code2}).}
\label{fig:code1}
\end{figure*}
...
\ \ \isakeyword{where}\ {\isacartoucheopen}apply{\isacharunderscore}ops\ ops\ {\isasymequiv}\ foldl\ {\isacharparenleft}{\isasymlambda}state\ oper{\isachardot}\ apply{\isacharunderscore}op\ oper\ state{\isacharparenright}\ {\isacharparenleft}{\isacharbrackleft}{\isacharbrackright}{\isacharcomma}\ {\isacharbraceleft}{\isacharbraceright}{\isacharparenright}\ ops{\isacartoucheclose}\isanewline
\isanewline
\isacommand{definition}\isamarkupfalse%
...
\end{isabellebody}
\caption{The move operation algorithm (continuation of Figure~\ref{fig:code1}).}
\label{fig:code2}
\end{figure*}
...
Figures~\ref{fig:code1} and~\ref{fig:code2} give the full source code for our algorithm in the Isabelle/HOL language~\cite{DBLP:conf/tphol/WenzelPN08}.
...
In this section we walk through the code in Figures~\ref{fig:code1} and~\ref{fig:code2} step by step, explaining the Isabelle/HOL syntax as we encounter it.
...
Thus, we define one kind of operation: \isa{Move t p m c} (Figure \ref{fig:code1}, lines 1--5).
...
This expression appears on lines 31 and 36 of Figure~\ref{fig:code1}, as we shall explain shortly.
...
The \isa{apply\_op} function (Figure~\ref{fig:code2}, lines 43--51) takes two arguments: a \isa{Move} operation to apply and the current replica state; and it returns the new replica state.
The constraint \isa{'t::\{linorder\}} in the type signature indicates that timestamps \isa{'t} are instances of \isa{linorder} type class, and they can therefore be compared with the \isa{<} operator defining a linear (or total) order.
...
The algorithm as specified in Figures~\ref{fig:code1} and~\ref{fig:code2} retains operations in the log indefinitely, so the memory use grows without bound.
...
% TODO \cite{SourceFiles}

To reason about the state of a replica we first define the function \isa{apply\_ops} on lines 53--54 of Figure~\ref{fig:code2}.
...
We then prove that \isa{unique\_parent tree} holds, where the \isa{unique\_parent} predicate is defined on lines 56--58 of Figure~\ref{fig:code2}: whenever the tree contains a triple whose third element is the child node \isa{c}, then the first and second elements of the triple (the parent node and the metadata) are uniquely defined.
...
\end{isabelle}
\noindent The \isa{acyclic} predicate is defined on lines 60--61 of Figure~\ref{fig:code2}, using the \isa{ancestor} relation (the transitive closure of the graph's edges): a graph contains no cycles if no node is an ancestor of itself.
...
\subsubsection{Other correctness properties}
...
To prove this theorem we must first show that \isa{undo\_op} performs the inverse of \isa{do\_op} and that every operation can be undone, that is:
\begin{isabelle}
...
This completes the proof that any two operations commute, provided that their timestamps are distinct from each other and from the contents of the log.
We can easily generalise this to show that any number of operations commute: given any sequence of operations, any permutation of that sequence will have the same effect, because we can produce the permutation by repeatedly swapping adjacent, commutative operations.
...
The details appear in our Isabelle theory files [URL omitted for blind review]. % \cite{SourceFiles}.
...
We therefore produce variants of the definitions in Figures~\ref{fig:code1} and~\ref{fig:code2} that are designed for execution rather than theorem proving.
...
\begin{figure}
  \includegraphics[width=10cm,keepaspectratio=true]{plot.pdf}
...
\begin{figure}
  \includegraphics[width=10cm,keepaspectratio=true]{leader-plot.pdf}
...
\begin{itemize}
    \item \citet{Martin:2010ih} define a CRDT for XML data, and \citet{Kleppmann:2016ve} define a CRDT for JSON.
    ...
    \item As discussed in \S\ref{sec:impossibility}, \citet{Najafzadeh:2017vk,Najafzadeh:2018bw} propose two implementations for a replicated filesystem: a CRDT in which conflicting moves are handled by duplicating tree nodes (as in Figures~\ref{fig:move-same-item}b and~\ref{fig:move-cycle}b), and a centralised implementation in which move operations must obtain a lock before executing (not a CRDT since it relies on synchronous coordination).
    \item \citet{AhmedNacer:2012us} outline approaches to handling conflicts on trees, but do not provide algorithms.
    \item \citet{Tao:2015gd} propose handling conflicting move operations by allowing the same object to appear in more than one location; thus, their datatype is strictly a DAG, not a tree.
...
\citet{Molli:2003cd} define an OT tree structure with a move operation.
...
\citet{Hughes:2016fp} test Dropbox and Google Drive against a formal specification, but they do not consider moving files, and thus they do not find the issue described in \S\ref{sec:move-cycle}.

\citet{Bjorner:2007hp} discusses the development of the Distributed File System Replication (DFS-R) component of Windows Server, during which a model checker found an issue with concurrent moves similar to Figure~\ref{fig:move-cycle}a.
...
Our approach of ordering operations by a timestamp, and undoing/redoing them as necessary so that they take effect in ascending timestamp order, is conceptually a very simple approach---although the details of the implementation are a bit subtle.
Similar ideas appear in many other systems, including the Bayou database~\cite{Terry:1995dn}, \citet{Jefferson:1985em}'s \emph{Time Warp} mechanism, \citet{Shapiro:2016hp}'s \emph{capricious total order}, and Burckhardt's \emph{standard conflict resolution}~\cite[{\S}4.3.3]{Burckhardt:2014hy}.
The concept of undo-redo is also well known from write-ahead logging~\cite{Mohan:1992fe,Mohan:1992hc}.
...
We are exploring applying undo-do-redo to other replication problems as well, and we expect that our formal verification approach will also benefit other applications of this principle.
