...
While previous work uses the kernel filesystem interface, Rumor is a userspace process that is invoked periodically by the user or by a deamon; when run, it compares the state of the replicas.
...
Unison is a file synchronisation tool with a formal specification~\cite{PierceVouillon:UnisonSpecTR} that allows two replicas to synchronise the state of a directory.
...
In this paper we have defined a novel algorithm that handles arbitrary concurrent modifications of a tree data structure---adding, moving, and removing nodes---in a peer-to-peer setting.
...
Updates made to a local replica take effect immediately, while operations from remote replicas can be applied in the background.
...
We also extracted a formally verified Scala implementation from our Isabelle/HOL definitions, and evaluated its performance across three replicas located in California, Ireland and Singapore.
Our implementation is able to apply local updates in under 1~ms, orders of magnitude faster than is possible with a central server or consensus protocol operating over these distances.
The current implementation is able to sustain up to 24 updates per second; we plan to develop a more scalable implementation in future work.
