...
As the rate of operations increases, the time between successive operations becomes smaller than the network delay between replicas; thus, there are more operations ``in flight'' at the same time, and more calls to \isa{undo\_op} and \isa{redo\_op} are required to order operations by timestamp.
...
The generated code could also serve as a reference implementation against which other implementations could be tested.
