...
\begin{tikzpicture}
  ...
  %%%
  \node at (9.85,4.04) {(\emph{a})};
  \node at (14.15,4.04) {(\emph{b})};
  \node at (9.85,1.04) {(\emph{c})};
  \node at (14.15,1.04) {(\emph{d})};
...
\begin{tikzpicture}
  ...
  \node [rectangle,draw] at (13.8,3) {
      ...
      \node at (0.95,0.0) {(\emph{b})};
      \node {$\mathsf{root}$} child {node (a2) {$A$} child {node {$B$}}} child {node {$B'$} child {node {$A'$} child {node (c2) {$C$}}}};
      \draw (a2) -- (c2);
      \end{tikzpicture}
  };
  ...
  \node [rectangle,draw] (right) at (13.8,0) {
      \begin{tikzpicture}[level distance=7mm]
      \useasboundingbox (-1.15,-2.3) rectangle (1.15,0.2);
      \node at (0.95,0.0) {(\emph{d})};
      \node {$\mathsf{root}$} child {node {$B$} child {node {$A$} child {node {$C$}}}};
      \end{tikzpicture}
  };
\end{tikzpicture}
\caption{Initially, $A$ and $B$ are siblings. $B$ is moved to be a child of $A$, while concurrently $A$ is moved to be a child of $B$. Boxes (\emph{a}) to (\emph{d}) show possible outcomes after the replicas have communicated and merged their states.}\label{fig:move-cycle}
...
\begin{figure*}
  ...
  \caption{Error message produced by Google Drive Backup and Sync on Mac OS as a result of performing the operations described in Section~\ref{sec:move-cycle}.}
...
\begin{itemize}
    \item In a filesystem, any file or directory can be moved to a different parent directory.
        Moreover, renaming a file is equivalent to moving the file to a new name without changing its parent directory.
    \item In a rich text editor, a paragraph node can be turned into a bullet point.
        This corresponds to creating a new bullet list node, creating a new bullet point node within it, and then moving the paragraph node to be a child of the new bullet point node.
...
In this section we demonstrate some of those problems by example, before proceeding to our solution in Section~\ref{sec:algorithm}.
