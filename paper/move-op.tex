...
\subsection{Commutativity of apply\_op}
...
We can now prove that lemma by induction over \isa{log}.

For the base case, where \isa{log} is empty, \isa{apply\_op oper2 (apply\_op oper1 (log, tree))} evaluates to first applying \isa{oper1} using \isa{do\_op}, then applying \isa{oper2} using \isa{do\_op}.
On the other hand, \isa{apply\_op oper1 (apply\_op oper2 (log, tree))} first applies \isa{oper2} using \isa{do\_op}, then undoes \isa{oper2} again to yield the original \isa{tree} (per \isa{do\_undo\_op\_inv}), then applies \isa{oper1} using \isa{do\_op}, and finally reapplies \isa{oper2} using \isa{redo\_op}.
The final states are the same on both sides of the equality.

For the inductive step, let the log be \isa{logop {\isacharhash} ops} (where \isa{\isacharhash} is the \emph{cons} operator), and let \isa{t3 = log\_time logop} be the timestamp of the operation at the head of the log.
...
Definitions making essential use of choice principles (such as our \isa{get\_parent} function) are obvious examples of definitions that will cause problems.
...
Within these definitions, we eschew the HOL \isa{set} type, and instead use a more efficient hash-map implementation provided by the Isabelle Collections Framework~\cite{XXX}.
...
Intuitively, this type can be thought of as a mapping from a \emph{child} node to its \emph{parent} node and associated metadata, if any such mapping exists.
...
\end{displaymath}
where $hm.lookup$ is the function that searches for a binding for key $\isa{c}$ in hash-map $t$, returning $\isa{Some}\ (m, p)$ when a binding from $c$ to $(m, p)$ exists in the hash-map, or $\isa{None}$ otherwise.
...
However, \isa{ancestor} is slightly different to our other definitions, as it is an inductively defined relation rather than a recursive function, and we therefore need further work to make this efficiently executable.
...
The correctness properties that we prove are best described using an example, as they all follow a similar pattern, and we therefore focus on the lemma \isa{efficient\_do\_op\_simulates} which states the following property:
...
Note that this property correctly captures our intuitive understanding of what it means for \isa{efficient\_apply\_ops} to be correct with respect to \isa{apply\_ops}.
...
Both follow as corollaries of the theorems \isa{apply\_ops\_commutes} and \text{apply\_ops\_acyclic}, respectively.
Note that the statement of \isa{efficient\_apply\_ops\_commutes} differs slightly from the statement of \isa{apply\_ops\_commutes}---the latter theorem asserts that identical sets are obtained whilst the former asserts that simply reading any key binding in $t$ gives the same result as reading in $u$.
Whilst this statement appears to be weaker than the statement of \text{apply\_ops\_acyclic}, it is correct, as the resulting hash-maps may not have an identical representation in-memory, depending on the particular ordering of key-value insertions, despite containing the same key-value bindings.
