...
\begin{itemize}
    ...
    \item To demonstrate its practical viability, we extract a formally verified Scala implementation of our algorithm from Isabelle.
        ...
        In a geo-replicated setup across three continents, our algorithm has approximately half the throughput of state machine replication, but up to five orders of magnitude lower latency.
...
\begin{figure}
  \begin{center}
  \includegraphics{../evaluation/data/crdt-generated.pdf}
  \end{center}
...
\begin{figure}
  \begin{center}
  \includegraphics{../evaluation/data/crdt-optimised.pdf}
  \end{center}
...
Our algorithm can be computationally expensive if a large number of undos and redos are required.
To demonstrate that this cost is acceptable in practice, we evaluated the performance of two different implementations of our algorithm.
The first implementation is Scala code extracted from our formally verified HOL definitions using Isabelle/HOL's code generation mechanism, as discussed in \S\ref{subsect.extracting}.
...
We wrapped both implementations in a simple network service and deployed three replicas of this service on Amazon EC2 \texttt{c5.large} instances in different regions worldwide: in Northern California (region \texttt{us-west-1}), Ireland (\texttt{eu-west-1}), and Singapore (\texttt{ap-southeast-1}).
...
\begin{figure*}
  \begin{center}
  \includegraphics{../evaluation/data/leader-vs-crdt.pdf}
  \end{center}
...
For remote operations, the execution time increases proportionally to the rate at which operations are generated: as the time interval between successive operations decreases relative to the network delay, there are more operations in flight at the same time, and more undos and redos are required to put operations in timestamp order.
...
The Isabelle-generated code is saturated at a rate of 600 operations/sec (with a median remote operation taking around 1~ms), while the optimised code is saturated at 5,700 operations/sec.
This factor of 9.5 performance difference is solely a result of inefficient code generation; functionally both implementations are performing the same work.
...
Besides the 59 lines of definitions given in Figure~\ref{fig:code}, our Isabelle/HOL formalisation consists of a further 2,495 lines of code.
...
Similar ideas appear in many other systems, including the Bayou database~\cite{Terry:1995dn}, Jefferson's \emph{Time Warp} mechanism~\cite{Jefferson:1985em}, Shapiro et al.'s \emph{capricious total order}~\cite{Shapiro:2016hp}, Burckhardt's \emph{standard conflict resolution}~\cite[{\S}4.3.3]{Burckhardt:2014hy}.
...
The formalisation is open source~\cite{SourceFiles} and included as supplementary data of this paper.
...
Martin Kleppmann is supported by a Leverhulme Trust Early Career Fellowship, the Isaac Newton Trust, Nokia Bell Labs, and crowdfunding supporters including Molham Aref, Henry Caushi, Chet Corcos, Gabriel Herrero-Beaumont, Bogdan Kanivets, David Pollak, SoftwareMill, Talent Formation Network, and Adam Wiggins.
