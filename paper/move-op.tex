...
Specifically, we say that a hash-map $\isa{t}$ of type $(\isa{'n}, \isa{'m} \times \isa{'n})\ \isa{hm}$ simulates a set $T$ of type $(\isa{'n} \times \isa{'m} \times \isa{'n})\ \isa{set}$:
\begin{displaymath}
\isa{hm.lookup}\ c t = \isa{Some}\ (m, p) \leftrightarrow (p, m, c) \in T \quad\text{for all $c$, $m$, and $p$}
\end{displaymath}
