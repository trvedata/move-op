...
In practice, the filename collision would be resolved by making the filenames distict, e.g.\ by appending a replica identifier to the filenames.
...
Isabelle/HOL can generate executable Haskell, OCaml, Scala, and Standard ML code from HOL definitions using a code generation mechanism~\cite{XXX}.
...
Using an explicit \emph{simulation relation}, we show the correctness of these efficient algorithms taking our abstract algorithms as ground truth of the correct behaviour.
Moreover, we show that our correctness theorems, such as \isa{apply\_ops\_acyclic}, easily transfer to the more efficient implementations.
