...
% Recommended options from acmart-pacmpl-template.tex
...
\label{fig:code1}
\Description{The move operation algorithm, implemented in the Isabelle/HOL language (continued in Figure~\ref{fig:code2}).}
\end{figure*}
...
\label{fig:code2}
\Description{The move operation algorithm (continuation of Figure~\ref{fig:code1}).}
\end{figure*}
...
This approach to determining ordering also easily supports reordering of child nodes within a parent: to move a node to a different position in a list, we use the list CRDT to generate a new ID at the desired position in the sequence.
...
\begin{figure}
  ...
  \label{fig:plots}
  \Description{Plot in which the x axis goes from 0 to 600 move operations per second, and the y axis goes from 0 to 2 milliseconds. Local operations take a near-constant time around 50 microseconds; the time for remote operations increases linearly from 0.2ms to 1.4ms.}
\end{figure}
...
\begin{figure}
  ...
  \Description{Local CRDT operations take about 50 microseconds, and reach saturation at 600 ops/sec. Leader-based operations take about 150--180ms, and reach saturation at 14,000 ops/sec.}
\end{figure}
