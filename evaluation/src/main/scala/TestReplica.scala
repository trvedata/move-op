...
object TestReplica {
  ...
  val MAX_PENDING_REQUESTS = if (USE_LEADER) 5000 else if (USE_GENERATED_CODE) 50 else 400
...
class ReplicaThread(replicaId: Long, metrics: MetricRegistry) extends Runnable {
  ...
  val undosRedos   = metrics.histogram("ReplicaThread.undosRedos")
  ...
  private[this] def processRequest(move: Protocol.Move, sender: ServerThread) {
    ...
    try {
      ...
      if (TestReplica.USE_LEADER) {
        // On the leader: replicate to all followers. On a follower: servers is empty.
        for (server <- servers) server.send(move)
      } else {
  ...
  private[this] def applyMoveOptimised(move: Protocol.Move) {
    var index = log.size
    var finished = false
    ...
    undosRedos.update(log.size - index - 1)
    ...
    // Truncate the log from time to time
    if (log.size > 1000000) {
      log.subList(0, log.size - 10000).clear()
    }
  }
